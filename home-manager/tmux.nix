...
in
{
  options.programs.tmux'.enable = lib.mkEnableOption "Enable tmux"; # Custom tmux module
  config = lib.mkIf config.programs.tmux'.enable {
    ...
    xdg.configFile."tmux/tmux.conf".text = ''
      ...
      set -g status-style "fg=${matu.on_surface},bg=${matu.surface_container}"
      set -g status-left-length 50
      set -g window-status-current-style "fg=${matu.primary},bg=${matu.surface_container}"
    '';
  };
}
